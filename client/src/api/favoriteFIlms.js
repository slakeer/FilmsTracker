...
export const removeFromFavorites = async (userId, movieId) => {
    try {
        const response = await fetch(`${API_URL}/favorite-movies/${userId}/${movieId}`, {
            method: 'DELETE',
            headers: {
                'Accept': '*/*',
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to remove from favorites: ${errorDetails.message || response.statusText}`);
        }
